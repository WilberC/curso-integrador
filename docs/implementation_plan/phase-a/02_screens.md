...
- [x] Layout: `JPanel` con `GridBagLayout` para centrar una tarjeta
- [x] Fondo del panel: `Theme.CANVAS_DARK`
- [x] Tarjeta centrada (`JPanel`, fondo `Theme.SURFACE_CARD`, radio `Theme.RADIUS_XL`, ancho ~400px):
...
- [x] Teclado: `Enter` en cualquier campo del formulario dispara el login (via `InputMap`/`ActionMap` sobre el `getRootPane()`)
- [x] La tarjeta usa `paintComponent` custom con `fillRoundRect` para el radio de esquina

---

## Pantalla 2: Dashboard (`ui/DashboardPanel.java`)

- [x] Layout: `BorderLayout`. NORTH: toolbar (timestamp + refresh). CENTER: contenido del dashboard
- [x] Contenido: `JPanel` con `GridLayout(2, 2, Theme.SP_LG, Theme.SP_LG)` con 4 `GaugeCard`
- [x] Debajo de los gauges (en un `JSplitPane` o segundo `JPanel`): `JTable` con los 10 lotes más urgentes
- [x] `DashboardPanel` inicialización:
  ...
  - Llama `GaugeCard.applyState(GaugeState)` según umbrales:
    ...
    - Resto → `SAFE`
- [x] Crear `component/GaugeCard.java` (ver `DESIGN.md` para implementación completa):
...
- [x] Auto-refresh: `javax.swing.Timer` cada 60 segundos llama `refreshDashboard()`:
  ```java
...
- [x] Crear `LoteTableModel extends AbstractTableModel`:
...
- [x] Row coloring via `prepareRenderer` override en la instancia de `JTable`:
...
- [x] Búsqueda: `DocumentListener` en el `JTextField` que llama `model.setData(filtered)` donde `filtered` filtra por `nombreProducto.contains(query)` y `numeroLote.contains(query)`

- [x] Botón "Nuevo Lote": abre `new NuevoLoteDialog(parentFrame)`, luego `dialog.setVisible(true)`. Al retornar (diálogo es modal), `model.setData(MockData.getLotes())` para refrescar

- [x] Shortcuts locales via `InputMap`/`ActionMap` en el panel:
...
- [x] Extiende `JDialog`, modal (`ModalityType.APPLICATION_MODAL`), `setResizable(false)`
- [x] Tamaño: 480×520px, centrado en el parent frame
- [x] Fondo: `Theme.SURFACE_CARD`
- [x] Layout del contenido (ver `DESIGN.md` para el patrón completo):
...
- [x] `Esc` cierra el diálogo (registrar en `getRootPane().getInputMap()`)
- [x] `Enter` en el último campo o cuando el botón "Registrar" tiene foco → guarda
...
- [x] Crear `AlertaTableModel extends AbstractTableModel`
...
- [x] `AlertasPanel` inicialización: carga `MockData.getAlertas()`, filtra solo `PENDIENTE` por defecto
- [x] Toggle "Mostrar todas": `JCheckBox` o `JToggleButton` en el toolbar, al cambiar llama `model.setData(filtered)`
- [x] Al "Atender": `alerta.setEstado(ATENDIDA)`, `model.fireTableRowsUpdated(row, row)`
- [x] Al "Ignorar": `alerta.setEstado(IGNORADA)`, `model.fireTableRowsUpdated(row, row)`
- [x] Shortcuts locales:
...
- [x] El nav item de Reportes en `SidebarPanel` se oculta (`setVisible(false)`) cuando `SessionManager.getCurrentUser().getRol() != SUPERVISOR`
- [x] Layout: `JSplitPane(HORIZONTAL_SPLIT)`:
...
- [x] Al generar: mostrar datos ficticios hardcoded en `MockData` que varíen según `TipoReporte`
